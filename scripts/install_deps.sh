...
#------------------------------------------------------------------------------
            ...
            Ubuntu|LinuxMint)
                ...
                #Ubuntu
                pkgs="build-essential git libboost-all-dev gcc g++"
                post_cmd=""
                case $(lsb_release -cs) in
                    trusty|qiana|rebecca|rafaela|rosa)
                        ...
                        pkgs=`echo $pkgs | sed s/libboost-all-dev//` # let Hunter build a newer version
                        pkgs="$pkgs cmake3 g++-8 gcc-8"
                        post_cmd="sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-8 80 --slave /usr/bin/g++ g++ /usr/bin/g++-8"
                        ;;
                    utopic)
